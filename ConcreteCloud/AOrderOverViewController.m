...
@interface AOrderOverViewController() <BMKMapViewDelegate>
...
@implementation AOrderOverViewController
...
- (void)getSiteList
{
    ...
    [[HttpClient shareClient] view:self.view post:URL_HZS_SITE parameters:param success:^(NSURLSessionDataTask *task, id responseObject) {
        
        HzsSiteListResponse *response = [[HzsSiteListResponse alloc] initWithDictionary:responseObject];
        
        NSArray<ProjectInfo *> *siteList = [response getHzsSiteInfo].siteList;
        
        NSArray<DTrackInfo *> *vehicleList = [response getHzsSiteInfo].vehicleList;
        
        [self markSites:siteList];
...
- (void)markSites:(NSArray<ProjectInfo *> *)siteList
{
    for (NSInteger i = 0; i < siteList.count; i++)
    {
        ProjectInfo *info = siteList[i];
        
        SiteMapAnnotation *ann = [[SiteMapAnnotation alloc] initWithLatitude:info.lat lng:info.lng];
        
        ann.info = info;
        
        ann.color = [self getTagColor:i];
        
        _dicColor[info.projectId] = ann.color;
...
- (void)markVehicles:(NSArray<DTrackInfo *> *)vehicleList
{
    for (NSInteger i = 0; i < vehicleList.count; i++)
    {
        ...
        ann.color = _dicColor[info.hzs_Order.siteId];
...
- (BMKAnnotationView *)mapView:(BMKMapView *)mapView viewForAnnotation:(id<BMKAnnotation>)annotation
{
    ...
    {
        SiteMapAnnotation *ann = (SiteMapAnnotation *)annotation;
        
        SiteAnnotationView *annView = (SiteAnnotationView *)[mapView dequeueReusableAnnotationViewWithIdentifier:@"site"];
        
        if (!annView)
        {
            annView = [[SiteAnnotationView alloc] initWithAnnotation:ann reuseIdentifier:@"site"];
        }
