...
@interface ChatController()<UIScrollViewDelegate, UIGestureRecognizerDelegate, AVAudioPlayerDelegate,
                AVAudioRecorderDelegate, PullTableViewDelegate>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([[dict objectForKey:@"senderName"] isEqualToString:[[Config shareConfig] getName]])
    {
        UILabel *lbName = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 80, 45, 70, 25)];
        lbName.text = [[Config shareConfig] getName];
...
- (void)getChatList
{
    ...
    param[@"rows"] = [NSNumber numberWithInt:10];
    param[@"hzsId"] = [[Config shareConfig] getBranchId];
    
    //[param setObject: forKey:];
    
    [[HttpClient shareClient] view:self.view post:URL_GET_CHAT_LIST parameters:param success:^(NSURLSessionDataTask *task, id responseObject) {
...
- (void)getMoreChatList
{
    ...
    param[@"rows"] = [NSNumber numberWithInt:10];
    param[@"hzsId"] = [[Config shareConfig] getBranchId];
    param[@"maxCode"] = [[_resultArray lastObject] objectForKey:@"code"];
...
- (void)sendText
{
    ...
    [[HttpClient shareClient] view:self.view post:URL_ADD_CHAT parameters:param success:^(NSURLSessionDataTask *task, id responseObject) {
        _tfContent.text = @"";
...
- (void)sendAudio
{
    ...
    param1[@"audio"] = base64Str;
    
    [[HttpClient shareClient] view:self.view post:URL_UPLOAD_MP3 parameters:param1 success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        param[@"content"] = url;
        param[@"userName"] = [[Config shareConfig] getName];
        param[@"timeLength"] = [NSNumber numberWithInteger:_countNum];
