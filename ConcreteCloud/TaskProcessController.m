...
#import "TaskProcessController.h"
...
@interface TaskProcessController()<TankerRelaxControllerDelegate, TankerOnWayControllerDelegate, TankerArrivedControllerDelegate,
...
@implementation TaskProcessController
...
#pragma mark - TankerRelaxControllerDelegate
...
- (void)onUploadVideo:(NSString *)url videoKey:(NSString *)videoKey
{
    ...
    [[HttpClient shareClient] view:self.view post:URL_A_CHECK_SPOT parameters:dic success:^(NSURLSessionDataTask *task, id responseObject) {
        [HUDClass showHUDWithLabel:@"现场视频上传成功" view:self.view];
        [self getUnfinishedTask];
    } failure:^(NSURLSessionDataTask *task, NSError *errr) {
